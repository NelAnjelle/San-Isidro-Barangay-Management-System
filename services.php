...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        $status = "Pending";
        $stmt = $conn->prepare("INSERT INTO service_requests (user_id, service_type, description, status) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("isss", $_SESSION['user_id'], $service_type, $description, $status);
        if ($stmt->execute()) {
...
$requests = [];
$stmt = $conn->prepare("SELECT service_type, description, status, created_at FROM service_requests WHERE user_id = ? ORDER BY created_at DESC");
$stmt->bind_param("i", $_SESSION['user_id']);
$stmt->execute();
$result = $stmt->get_result();
...
<head>
    ...
    <title>Service Requests - Barangay San Isidro Management System</title>
...
<body class="bg-gray-100 min-h-screen flex flex-col">
    ...
    <div class="container mx-auto mt-8 px-4 flex-grow">
        <div class="bg-white p-8 rounded-lg shadow-lg">
            <h2 class="text-3xl font-bold text-gray-800 mb-6 text-center">Request a Service</h2>
            <?php if (!empty($errors)): ?>
            ...
            <form method="POST" action="" class="space-y-6">
                <div>
                    ...
                    <input type="text" id="service_type" name="service_type" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                </div>
                <div>
            ...
            <div class="mt-8">
                <h3 class="text-2xl font-semibold text-gray-800 mb-4">Your Service Requests</h3>
